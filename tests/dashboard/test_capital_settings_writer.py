...
CURRENT = {
    ...
    ("futures", "allocated_balance"): 10_000.0,
    ("futures", "minimum_capital_per_trade"): 10.0,
    ("futures", "maximum_capital_per_trade"): 1_000.0,
    ("futures", "leverage_ceiling"): 1.0,
}
...
def test_money_mode_is_refused_however_valid_it_looks():
    ...
    verdict = judge_change("futures", "money_mode", "live", CURRENT)
...
def test_a_setting_nobody_listed_is_refused():
    ...
    for name in ("quote_currency", "captured_symbol_count", "journal_path", "anything"):
        verdict = judge_change("futures", name, 5.0, CURRENT)
        assert not verdict.is_accepted
...
def test_a_value_that_is_not_a_number_is_refused_rather_than_zeroed():
    for value in ("", "abc", None, [], {}, "1.2.3"):
        verdict = judge_change("futures", "allocated_balance", value, CURRENT)
...
def test_true_is_not_a_number_even_though_python_says_it_is():
    ...
    assert not judge_change("futures", "leverage_ceiling", True, CURRENT).is_accepted


def test_a_negative_capital_setting_is_refused():
    verdict = judge_change("futures", "allocated_balance", -1.0, CURRENT)
    assert not verdict.is_accepted
...
def test_a_value_that_is_already_set_is_refused_rather_than_rewritten():
    ...
    verdict = judge_change("futures", "allocated_balance", 10_000.0, CURRENT)
    assert not verdict.is_accepted
...
def test_a_minimum_above_the_maximum_is_refused():
    verdict = judge_change("futures", "minimum_capital_per_trade", 5_000.0, CURRENT)
    assert not verdict.is_accepted
...
def test_a_maximum_above_the_allocation_is_refused():
    verdict = judge_change("futures", "maximum_capital_per_trade", 50_000.0, CURRENT)
    assert not verdict.is_accepted
...
def test_an_allocation_above_the_main_balance_is_refused():
    ...
    verdict = judge_change("futures", "allocated_balance", 50_000.0, CURRENT)
    assert not verdict.is_accepted
...
def test_a_leverage_ceiling_below_unlevered_is_refused():
    verdict = judge_change("futures", "leverage_ceiling", 0.5, CURRENT)
    assert not verdict.is_accepted
...
def test_a_change_that_is_individually_fine_and_jointly_wrong_is_caught():
    ...
    assert find_contradictions({
        ("futures", "minimum_capital_per_trade"): 900.0,
        ("futures", "maximum_capital_per_trade"): 1_000.0,
        ("futures", "allocated_balance"): 500.0,
    })
...
def test_a_safe_change_is_accepted():
    verdict = judge_change("futures", "maximum_capital_per_trade", 500.0, CURRENT)
...
def test_raising_leverage_within_the_rules_is_accepted():
    ...
    for ceiling in (2.0, 5.0, 10.0, 20.0):
        assert judge_change("futures", "leverage_ceiling", ceiling, CURRENT).is_accepted
...
'''
    (root / "segments" / "futures.toml").write_text(original)
    monkeypatch.setattr(
        "runtime.settings_reader.settings_directory", lambda: root
    )

    write_setting("futures", "allocated_balance", 5_000.0, changed_by="a test")
    written = (root / "segments" / "futures.toml").read_text()
...
def test_writing_refuses_a_section_that_does_not_exist(tmp_path, monkeypatch):
    root = tmp_path / "settings"
    (root / "segments").mkdir(parents=True)
    (root / "segments" / "futures.toml").write_text("[something_else]\nvalue = 1\n")
    monkeypatch.setattr("runtime.settings_reader.settings_directory", lambda: root)

    with pytest.raises(KeyError):
        write_setting("futures", "allocated_balance", 5_000.0)
...
def test_writing_refuses_an_entry_with_no_note(tmp_path, monkeypatch):
    ...
    root = tmp_path / "settings"
    (root / "segments").mkdir(parents=True)
    (root / "segments" / "futures.toml").write_text(
        '[allocated_balance]\nvalue = 10000.0\nunit  = "USDT"\n'
    )
    monkeypatch.setattr("runtime.settings_reader.settings_directory", lambda: root)
...
def test_the_written_file_is_still_readable_by_the_settings_reader(tmp_path, monkeypatch):
    ...
    write_setting("futures", "allocated_balance", 2_500.0, changed_by="a test")

    document = load_settings_document(root / "segments" / "futures.toml", "futures")
