...
def describe_all_captures(readers: dict) -> dict:
    ...
    standing keeps only top-level numbers -- so the merge carries each venue's
    facts under a suffixed key and the totals under the plain one. Written for
    read_standing, which reported {} for this part while its per-venue detail
    had nowhere that fits (2026-08-24).
    """
